...
- 調整項目：在模板中新增項目（中英文），並可使用 ▲/▼ 調整拍照順序。
  Manage Items: Add items (CN/EN) and use ▲/▼ to reorder them.

B. 拍照作業 (Taking Photos)
- 選擇模板：在首頁輸入資料時，必須選擇適用的「模板」。
  Select Template: Select a "Template" before starting.
- 拍照/選取：按順序進行拍照，支援「是/否」特殊選擇（如：是否需要轉換膠框）。
  Capture: Follow steps to take photos. Supports "Yes/No" choices if configured.
- 項目備註：每一項拍照時皆可輸入獨立的「項目備註」。
  Item Note: Each step allow entering an individual note.

C. 查詢與匯出 (Search & Export)
- 備份管理：使用管理按鈕匯出 JSON 備份檔。
  Backup: Export JSON backup files.
...
- 離線使用：系統支援離線操作，但匯出備份時需確保資料已儲存。
- 更新提示：若看到「發現新版本」，請點擊確定以獲取最新功能。
"@

$selection.TypeText($content)
$filename = Join-Path (Get-Location) "User_Manual_v1.9.0.docx"
$doc.SaveAs($filename)
$doc.Close()
$word.Quit()
